...
def test_model_select_imports_safely(
    ...
    """
    Ensures that importing mlops_project.model_select does NOT crash.
    All Mlflow and file I/O operations are mocked.
    """

    mock_mlflow.get_experiment_by_name.return_value = MagicMock(experiment_id=123)

    mock_df = MagicMock()
    mock_df.iloc.__getitem__.return_value = {"metrics.f1_score": 0.9, "run_id": "12345"}
    mock_mlflow.search_runs.return_value = mock_df

    mock_open.return_value.__enter__.return_value.read.return_value = '{"modelA":{"weighted avg":{"f1-score":0.7}}}'
    mock_pd.DataFrame.return_value = MagicMock(
        sort_values=MagicMock(
            return_value=MagicMock(iloc=MagicMock(return_value=MagicMock(name="modelA")))
        )
    )
    ...
    mock_mlflow.get_run.return_value = (None, {"metrics": {"f1_score": 0.5}})
